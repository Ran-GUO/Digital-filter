...
void VectorizedDecimation::do_horizontal_decimation(uint16_t *dst_ptr, uint16_t *src_ptr,
                           uint dst_width, uint dst_height,
  ...
  uint i_begin,i_end;	
    ...
    i_begin=i_*dst_height/max_thread_;
    i_end=(i_+1) *dst_height/max_thread_;
...
void VectorizedDecimation::do_vertical_decimation(uint16_t *dst_ptr, uint16_t *src_ptr,
                    uint dst_width, uint dst_height,
                    uint dst_stride, uint src_stride,uint i_,uint max_thread_) {
	uint16_t *src_row0 = src_ptr;
	uint16_t *dst_row0 = dst_ptr;
   ...
   uint i_begin,i_end;	
    ...
    i_begin=i_*dst_width/max_thread_;
    i_end=(i_+1) *dst_width/max_thread_;
  ...
  for (uint x = i_begin; x < i_end; ++x) {
    ...
    for (uint y = 0; y < dst_height; ++y) {
		...
		//condition aux limites 
		if (idy<0) idy=0; //dessus
		if (idy>static_cast<int> (dst_height)) idy=dst_height;//dessous
		d[k+3]=static_cast<int16_t>(src_col[idy]);
