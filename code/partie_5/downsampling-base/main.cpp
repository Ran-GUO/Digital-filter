...
void TopConfig::main() {
  ...
  YuvImage input_image(file_name_, width_, height_,poly_ptr);
  poly_ptr->print_method();
  ...
  //  measure the time and start doswnsizing loop
...
//thread
 ...
 // result_image.downsize_from(input_image,0, 2);
 // result_image.downsize_from(input_image,1, 2);

  auto time_end = std::chrono::high_resolution_clock::now();
  auto elapsed_time_ms = std::chrono::duration_cast<std::chrono::milliseconds>(time_end - time_start);
...
void TopConfig::print_help() const {
  ...
  std::cout << "--methods " << CSI("italic") << "string" << CSI("normal") << "  Choices:simple,convolution or vectorized(default :simple)" << std::endl;
}
...
TopConfig::TopConfig(const int argc, const char *const argv[]) {
  ...
  if (argc > 1) {
    ...
    do {
	  ...
	  //methods
	  else if (current_arg== "--methods"){
		idx++;
		  if (idx >= argc) {
          std::cerr << "Error: argument " << current_arg << " needs a parameter" << std::endl;
          exit(-1);
        }
		  std::string methods_arg(argv[idx]);

		 if (methods_arg == "simple") {
			poly_ptr=new SimpleDecimation(max_thread_);	
	  }
	  	 else if (methods_arg == "convolution") {
			poly_ptr=new ConvolutionDecimation(max_thread_);
	  }
	  	 else if (methods_arg == "vectorized") {
			poly_ptr=new VectorizedDecimation(max_thread_);
		 }			
		else{
			std::cerr << "Error: argument " << methods_arg << " is not a method" << std::endl;
			exit(-1);
		}	
