...
void YuvComponent::downsize_from(const YuvComponent &src) {
  ...
  src.poly_ptr->horizontal_decimation(
      tmp.ptr_, src.ptr_,
      width_, src.height_,
      tmp.stride_, src.stride_);
  src.poly_ptr->vertical_decimation(
      ptr_, tmp.ptr_,
      width_, height_,
      stride_, tmp.stride_);
}
...
void YuvImage::downsize_from(const YuvImage &src_image, const uint max_thread) {
  for (uint i = 0; i < components_.size(); ++i) {
    components_[i].downsize_from(src_image.components_[i], max_thread);
  }
}
...
void YuvImage::downsize_threaded_by_component(const YuvImage &src_image) {
  std::thread t_y([&]() {
    components_[0].downsize_from(src_image.components_[0]);
  });
  std::thread t_u([&]() {
    components_[1].downsize_from(src_image.components_[1]);
  });

  components_[2].downsize_from(src_image.components_[2]);
