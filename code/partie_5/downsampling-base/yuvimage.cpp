...
YuvComponent::YuvComponent() {
  ...
  poly_ptr=new SimpleDecimation(nbThread);
...
YuvComponent::YuvComponent(const uint width, const uint height, Decimation *p) {
  //
  if (enable_trace) {
...
//! Non threaded
//
void YuvImage::downsize_from(const YuvImage &src_image,const uint i_,const uint max_thread) const{
  for (uint i = 0; i < components_.size(); ++i) {
    components_[i].downsize_from(src_image.components_[i],i_, max_thread);
  }
}
...
//!
void YuvImage::downsize_threaded_by_component(const YuvImage &src_image,uint i_,uint max_thread_) const{
  std::thread t_y([&]() {
    components_[0].downsize_from(src_image.components_[0],i_,max_thread_);
  });
  std::thread t_u([&]() {
    components_[1].downsize_from(src_image.components_[1],i_,max_thread_);
  });

  components_[2].downsize_from(src_image.components_[2],i_,max_thread_);
...
void YuvImage::write(const std::string &file_name) const{
...
YuvImage::YuvImage(const std::string &file_name, const uint width, const uint height,Decimation *p) {
  ...
  try {
   ...
   poly_ptr=p;
    ...
    components_.emplace_back(width_, height_,poly_ptr);  // luma
    components_.emplace_back(width_ >> 1, height_ >> 1,poly_ptr);  // cb
    components_.emplace_back(width_ >> 1, height_ >> 1,poly_ptr);  // cr
